...
const ScheduleManagement: React.FC = () => {
  ...
  useEffect(() => {
    console.log("📅 Schedule params changed:", {
      role,
      fromDate,
      toDate,
      viewMode,
  ...
  useRealtimeSchedule(
    stableRealtimeParams.role,
    stableRealtimeParams.fromDate,
    stableRealtimeParams.toDate
  );
  ...
  const checkScheduleConflict = useCallback(
    (day: string, timeSlotId: string, roomId: string) => {
  ...
  const handleAddSchedule = useCallback(
    async (day: string, timeSlotId: string, userId: string, roomId: string) => {
      try {
        if (checkScheduleConflict(day, timeSlotId, roomId)) {
          const timeSlot = timeSlots.find((ts) => ts.id === timeSlotId);
          toast({
            title: "Xung đột lịch làm việc",
            description: `Phòng này đã có người làm việc trong ${
              timeSlot?.name || timeSlotId
            } này`,
            variant: "warning",
          });
          return false;
      ...
      const loadAllData = async () => {
        try {
          ...
          if (currentParamsRef.current !== paramsKey) {
            console.log(
              `Loading schedules for ${role} from ${fromDate} to ${toDate}`
            );
    ...
    const fetchStaffAndRooms = async () => {
      ...
      try {
        ...
        const message =
          err?.response?.data?.Message ||
          err?.message ||
          "Không thể tải danh sách nhân viên hoặc phòng";
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4 mt-2">
          ...
          {statsLoading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              <div className="text-center">
                <div className="text-xl font-bold text-clinic-navy">
                ...
                <div className="flex-1 overflow-auto">
                  <Table className="min-w-full">
                    <TableHeader className="sticky top-0 bg-white z-10">
                      <TableRow>
                        <TableHead className="w-28 bg-white text-xs">
                          Thời gian
                        </TableHead>
                        {weekDays.map((day) => (
                          <TableHead
                            key={day.id}
                            className="text-center min-w-28 bg-white text-xs"
                          >
                            <div className="p-1">
                        ...
                        <TableRow key={timeSlot.id}>
                          <TableCell className="font-medium sticky left-0 bg-white z-10 p-2">
                            <div className="flex items-center space-x-1">
                              ...
                              <div>
                                <div className="font-semibold text-clinic-navy text-xs">
                                  {timeSlot.name}
                                </div>
                                <div className="text-xs text-gray-500">
                            ...
                            >
                              <div className="space-y-1">
                                <div className="space-y-1 min-h-[50px]">
                                  {getScheduleForDay(day.date, timeSlot.id).map(
                                    (schedule, index) => (
                                      <StaffItem
                      ...
                      <div className="grid grid-cols-7 gap-1">
                        {["CN", "T2", "T3", "T4", "T5", "T6", "T7"].map(
                          (dayName) => (
                            <div
                              key={dayName}
                              className="p-1 text-center text-xs font-medium text-gray-600 bg-gray-50 rounded"
                            >
                              {dayName}
                            </div>
                          )
          ...
          <div className="bg-white rounded-lg p-4 w-full max-w-sm">
            ...
            <div className="mb-3 p-2 bg-yellow-50 border border-yellow-200 rounded">
              ...
              <p className="text-xs text-yellow-700 mt-1">
                Mỗi phòng chỉ có thể có một {getStaffTypeName()} trong một ca
                làm việc.
              </p>
            </div>
                ...
                >
                  ...
                  {staffOptions
                    .filter((staff) => {
                      ...
                      const scheduledUserIds = getScheduleForDay(
                        selectedDay,
                        selectedShift
                      ).map((s) => s.userId);
              ...
              <div>
                ...
                >
                  ...
                  {roomOptions.map((room) => {
                    const isOccupied = checkScheduleConflict(
                      selectedDay,
                      selectedShift,
                      room.id
                    );
      ...
      {editingSchedule && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-4 w-full max-w-sm">
            ...
            <p className="text-gray-600 mb-3 text-sm">
              {getStaffName(editingSchedule.userId)} -{" "}
              {
                timeSlots.find((ts) => ts.id === editingSchedule.timeSlotId)
                  ?.name
              }
            </p>
