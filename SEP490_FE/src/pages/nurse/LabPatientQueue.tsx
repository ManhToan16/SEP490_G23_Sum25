...
const LabPatientQueue: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleAssignmentChanged = (assignmentData: any) => {
      console.log('🔔 [LabPatientQueue] Received AssignmentChanged event:', assignmentData);
      ...
      console.log('🔄 [LabPatientQueue] Normalized data:', normalizedData);
      console.log('🏠 [LabPatientQueue] Current room ID:', currentRoomId);
      
      // Chỉ xử lý nếu assignment thuộc về phòng xét nghiệm hiện tại
      if (normalizedData.LaboratoryRoomId === currentRoomId) {
        console.log('✅ [LabPatientQueue] Assignment belongs to current room, processing...');
        ...
        if (normalizedData.Action === 'CREATE') {
          ...
          console.log('🔄 [LabPatientQueue] Updating existing assignment');
          console.log('🔍 [LabPatientQueue] Looking for assignment ID:', normalizedData.AssignmentId);
          console.log('📋 [LabPatientQueue] Current assignments:', assignments.map(a => ({ id: a.id, assignmentId: a.assignmentId, status: a.status })));
          ...
          setAssignments(prev => {
            const updated = prev.map(assignment => {
              ...
              if (isMatch) {
                console.log('✅ [LabPatientQueue] Found matching assignment, updating status from', assignment.status, 'to', normalizedData.Status);
                return { ...assignment, status: normalizedData.Status };
              }
              return assignment;
            });
            
            console.log('📋 [LabPatientQueue] Updated assignments:', updated.map(a => ({ id: a.id, assignmentId: a.assignmentId, status: a.status })));
            return updated;
          });
          
          // Hiển thị thông báo real-time với thông tin chi tiết hơn
          const statusText = normalizedData.Status === 'COMPLETED' ? 'Hoàn thành' :
                           normalizedData.Status === 'IN_LABORATORY_PROGRESS' ? 'Đang xét nghiệm' :
          ...
          setRealtimeNotification({
            show: true,
            message: `🔔 Kỹ thuật viên cập nhật: ${normalizedData.PatientName} - ${statusText}`,
            type: 'success'
          });
          ...
          setTimeout(() => {
            console.log('🔄 [LabPatientQueue] Force refreshing data after SignalR update');
            fetchLabScheduleAndAssignments(selectedDate);
          }, 1000);
        }
      } else {
        console.log('❌ [LabPatientQueue] Assignment does not belong to current room, ignoring');
      }
    };
  ...
  const handleCallPatient = async (visit: any) => {
    try {
      console.log('📞 [LabPatientQueue] Calling patient:', {
      ...
      console.log('✅ [LabPatientQueue] API call successful:', response);
      ...
      console.error('❌ [LabPatientQueue] Error calling patient:', error);
      toast({
        title: "Lỗi!",
