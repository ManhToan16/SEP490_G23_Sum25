...
namespace Test2.Services.ServicesTest
{
    ...
    public class DeleteServiceTests
    {
        ...
        [Test]
        public async Task Delete_ExistingServiceId_DeletesSuccessfully()
        {
            // Arrange
            ...
            // Act
            await _service.Delete(serviceId);
        ...
        [Test]
        public void Delete_NonExistentServiceId_ThrowsResourceNotFoundException()
        {
            // Arrange
            var serviceId = Guid.NewGuid().ToString();
            ...
            // Act + Assert
            var ex = Assert.ThrowsAsync<ResourceNotFoundException>(() => _service.Delete(serviceId));
            Assert.That(ex.Message, Is.EqualTo("Không tìm thấy dịch vụ."));

            _serviceRepositoryMock.Verify(r => r.DeleteAsync(It.IsAny<Service>()), Times.Never());
