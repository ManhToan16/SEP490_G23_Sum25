...
{
    [TestFixture]
    public class GetLaboratoryResultTest
    {
        ...
        [Test]
        public async Task GetById_ShouldReturnDto_WhenResultExists()
        {
            // Arrange
            var resultId = "lr1";
            var labResult = new LaboratoryResult
            {
                ...
                AssignmentId = "asg1",
                TechnicianId = "tech1",
                ExaminationResultId = "er1",
                CreatedAt = DateTime.UtcNow
            };

            _resultRepoMock.Setup(r => r.GetByIdAsync(resultId)).ReturnsAsync(labResult);

            // Act
            var dto = await _service.GetById(resultId);

            // Assert
            Assert.NotNull(dto);
            Assert.AreEqual(resultId, dto.Id);
            Assert.AreEqual("asg1", dto.AssignmentId);
        }

        [Test]
        public void GetById_ShouldThrow_WhenResultNotFound()
        {
            // Arrange
            var resultId = "not-exist";
            _resultRepoMock.Setup(r => r.GetByIdAsync(resultId)).ReturnsAsync((LaboratoryResult?)null);

            // Act & Assert
            var ex = Assert.ThrowsAsync<ResourceNotFoundException>(() => _service.GetById(resultId));
