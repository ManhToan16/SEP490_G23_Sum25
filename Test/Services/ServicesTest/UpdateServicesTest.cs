...
namespace Test.Services.ServicesTest
{
    ...
    public class ServiceServiceUpdateTests
    {
        ...
        [Test]
        public async Task TC007_Update_UppercaseName_ReturnsSuccess()
        {
            var id = Guid.NewGuid().ToString();
            var existingService = new Service { Id = id, LaboratoryRoomsId = Guid.NewGuid().ToString(), Name = "Dịch vụ cũ", Price = 100, Description = "Mô tả cũ" };
            var request = new UpdateServiceDTO
            ...
            _serviceRepositoryMock
                .Setup(r => r.ExistsByNameAsync(request.Name, existingService.LaboratoryRoomsId))
                .ReturnsAsync(false);
        ...
        public async Task TC010_Update_PriceNull_ReturnsSuccess()
        {
            ...
            _serviceRepositoryMock
                .Setup(r => r.ExistsByNameAsync(existingService.Name, existingService.LaboratoryRoomsId))
                .ReturnsAsync(false);
